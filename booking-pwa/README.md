...
> All screens use the Tactical Command dark theme.

| Bookings | Dashboard | Add Booking |
|----------|-----------|-------------|
| Cards with urgency stripes, location tabs, bullet remarks, and flag tags | Stats, reminders, and by-location counts | Free-text inputs, tripartite dates, checkbox flags, bulleted remarks |
...
| Token | Hex | Usage |
|-------|-----|-------|
| `--ds-canvas` | `#07101e` | Page / scaffold background |
| `--ds-surface` | `#0d1a2e` | Cards, panels, sheets |
| `--ds-raised` | `#122234` | Inputs, elevated elements |
| `--ds-overlay` | `#1c3050` | Hover backgrounds |
| `--ds-border` | `#1c3050` | Default borders |
| `--ds-accent` | `#3d82f0` | Brand / primary actions |
| `--ds-text-hi` | `#e8f1fa` | Primary content |
| `--ds-text-lo` | `#7990a8` | Labels, metadata |
| `--ds-text-dim` | `#445872` | Placeholders, disabled |
| `--ds-red` | `#e84040` | Urgent / error |
| `--ds-amber` | `#f0980a` | Warning / tomorrow |
| `--ds-green` | `#1ec25a` | Normal / success |
| `--ds-sky` | `#38b2f8` | Helicopter tag |
| `--ds-violet` | `#a78bfa` | Assistance tag |

### Flutter migration

Every token maps to a Flutter `ColorScheme` role:

```dart
// Flutter ThemeData.colorScheme equivalents
scaffoldBackgroundColor → --ds-canvas
surface                 → --ds-surface
surfaceVariant          → --ds-raised
outline                 → --ds-border
primary                 → --ds-accent
onPrimary               → #ffffff
onSurface               → --ds-text-hi
onSurfaceVariant        → --ds-text-lo
error                   → --ds-red
```
...
- `locationId: 'shimla'` → `location: 'Shimla'`
- `remarks: 'some string'` → `remarks: ['some string']`
- `customFlags` added with `[]`
...
const { isListening, interimText, isSupported, start, stop } = useVoiceInput({
  onResult: (transcript) => { /* parse and fill form fields */ }
})
```

The `parseBookingCommand(text, config)` NLP function extracts:
...
| Nights | `"3 nights"`, `"two nights"` |
| Check-in | `"today"`, `"tomorrow"` |
| Helicopter | `"helicopter"`, `"heli"` |
| Assistance | `"assistance"`, `"wheelchair"` |

**Browser support:** Chrome, Edge, Safari (iOS 14.5+). Falls back gracefully — the mic button is hidden when the API is unavailable.

---

## PWA & Offline

The service worker is built in `injectManifest` mode via `vite-plugin-pwa`. It precaches the app shell (HTML, JS, CSS) so the app opens instantly without a network connection. Booking data is stored in `localStorage` and never requires a server.
